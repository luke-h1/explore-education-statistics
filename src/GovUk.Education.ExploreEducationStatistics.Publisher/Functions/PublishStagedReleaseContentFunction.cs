            ...
            ILogger logger)
        {
            ...
            if (scheduled.Any())
            {
                ...
                try
                {
                    ...
                    var releaseIdsToSendSubscriberEmails = _contentDbContext.Releases
                        .Include(r => r.ReleaseStatuses)
                        .Where(r => releaseIds.Contains(r.Id))
                        .ToList()
                        .Where(r => r.EmailSubscribers)
                        .Select(r => r.Id)
                        .ToArray();
                    await _notificationsService.NotifySubscribers(releaseIdsToSendSubscriberEmails);
