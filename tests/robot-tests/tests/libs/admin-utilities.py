...
def data_csv_number_contains_xpath(num, xpath):
    try:
        elem = sl.driver.find_element('xpath', f'//*[@id="dataFileUploadForm"]/dl[{num}]')
    except BaseException:
        raise_assertion_error(f'Cannot find data file number "{num}"')
    try:
        elem.find_element('xpath', xpath)
    except BaseException:
...
def data_file_number_contains_xpath(num, xpath):
    try:
        elem = sl.driver.find_element('xpath', f'//*[@id="fileUploadForm"]/dl[{num}]')
    except BaseException:
        raise_assertion_error(f'Cannot find data file number "{num}"')
    try:
        elem.find_element('xpath', xpath)
...
def user_waits_for_release_process_status_to_be(status, timeout):
    ...
    while time.time() < max_time:
        try:
            sl.driver.find_element('css selector', f'#release-process-status-Failed')
            raise_assertion_error('Release process status FAILED!')
        except BaseException:
            pass
        try:
            sl.driver.find_element('css selector', f'#release-process-status-{status}')
            return
        except BaseException:
