...
def user_checks_number_of_other_releases_is_correct(number):
    elems = sl.driver.find_elements('xpath', '(.//*[@data-testid="other-release-item"])')
...
def get_methodology_link(topic, publication, methodology):
    try:
        sl.driver.find_element('xpath', f'//summary/span[text()="{topic}"]')
    except BaseException:
    ...
    try:
        return sl.driver.find_element('xpath',
...
def user_checks_generated_permalink_is_valid():
    elem = sl.driver.find_element('css selector', '[data-testid="permalink-generated-url"]')
